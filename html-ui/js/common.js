...
var consumer_url = 'http://localhost:32144/customers/consumer/'

var producer_url = 'http://localhost:32144/customers/producer/' // cart api (? since it will publish to this)
...
function getCartsByCustomerId()
    ...
    */
    function checkout(e){
		...
		let all_products;
		// get list of items in the cart (if we're following the diagram?)
		axios.get(consumer_url)
	    .then(response => {
	        ...
	        console.log(`GET list of product from cart`, products);
	    })
	    .catch(error => console.error(error));


		// submit list of products to be carted (this is the cart service??)
		axios.post(producer_url, all_products) //get all items from the get service
    	.then(response => {
			const added = response.data;
        	console.log(`Successfully addedd`, added); // the current app does not return the list of ids that were successfully added
